...
export default function Chat() {
  ...
  const handleSend = () => {
    ...
    if (BLOCKED_KEYWORDS.some(k => lower.includes(k))) {
      toast({ title: "Message Blocked", description: "Safety Alert: Discussion of payments or sales is prohibited.", variant: "destructive" });
      return;
    }
        ...
        {selectedMatchId && activeMatch ? (
          <>
            ...
            <div className="p-4 bg-white border-t flex gap-2">
              <Input placeholder="Type a message..." value={msgInput} onChange={e => setMsgInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} className="flex-1 rounded-full bg-muted/30 border-none focus-visible:ring-1" />
              <Button size="icon" className="rounded-full" onClick={handleSend} disabled={!msgInput.trim()}><Send size={18} /></Button>
            </div>
          </>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground opacity-50"><MessageCircle size={64} className="mb-4" /><p>Select a match to start chatting</p></div>
        )}
      </Card>
    </div>
  );
